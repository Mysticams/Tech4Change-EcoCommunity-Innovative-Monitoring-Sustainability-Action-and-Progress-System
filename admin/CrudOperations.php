...
class Project
{
    ...
    // no filtering by deleted column
    ...
    // List all projects even deleted ones
    ...
    public function exportProgressReportAsCSV()
    {
        ...
        $query = "SELECT title, description, progress, status FROM {$this->table} 
        WHERE status != 'recently_deleted'";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
