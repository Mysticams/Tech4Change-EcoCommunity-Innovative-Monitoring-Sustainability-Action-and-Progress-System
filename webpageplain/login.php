...
class User
{
    ...
    public function __construct($username, $password, $role = "", $email = "")
    {
        ...
        $stmt = $con->prepare("SELECT * FROM user WHERE `username`=? AND 
        `password`=? LIMIT 1");
    ...
    public function RegisterUser()
    {
        ...
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO `user`(`username`, `password`, `email`, `role`)
             VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $this->username,
                $this->password,
...
<body>
        ...
        <div id="loginFormContainer">
            ...
            <form id="loginForm" method="POST">
                ...
                <button type="button" id="loginButton"
                    style="padding: 10px 20px; background-color: #AFE1AF; 
                color: #006400; border: none; border-radius: 5px; font-size: 1em;">Login</button>
            </form>
    ...
    <script>
        ...
        document.getElementById('loginButton').onclick = function() {
            ...
            if (role === 'admin') {
                form.action = '../admin/adminMain.php';
            } else if (role === 'user') {
                form.action = '../user/userDashboard.php';
            } else {
